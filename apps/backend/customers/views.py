...
class DriverProfileUpdateView(APIView):
    ...
    def patch(self, request):
        ...
        if serializer.is_valid():
            ...
            if was_offline and profile.is_online:
                ...
                self._redispatch_nearby_tasks(profile)
    ...
    def _redispatch_nearby_tasks(self, profile):
...
class DriverLocationUpdateView(APIView):
    ...
    def post(self, request):
        ...
        if created and driver_profile.is_online:
            self._redispatch_nearby_tasks(driver_profile, loc)

        return Response({'message': 'Location updated'}, status=status.HTTP_200_OK)

    def _redispatch_nearby_tasks(self, profile, location):
        from math import radians, cos, sin, asin, sqrt
        def haversine(lat1, lon1, lat2, lon2):
            lat1, lon1, lat2, lon2 = map(radians, [lat1, lon1, lat2, lon2])
            dlon = lon2 - lon1
